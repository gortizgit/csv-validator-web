...
class MembershipsDailyMaintenanceValidator:
    ...
    def validate(self, domo_df: pd.DataFrame, inf_df: pd.DataFrame) -> ValidationRun:
        ...
        print("  [9/11] Duplicate behavior...")
        self._validate_duplicate_behavior(domo_df, inf_df)
    ...
    def _validate_country_scope(self, domo_df: pd.DataFrame, inf_df: pd.DataFrame) -> None:
        ...
        if missing_country_cols:
            self.add_check(
                "MEM-DAILY-006A",
                "Verify strict country validation prerequisites exist",
                "FAIL",
                "Country Content",
        ...
        for idx, country in enumerate(COUNTRY_CANONICAL_CODES, start=1):
            ...
            self.add_check(
                f"MEM-DAILY-{country}-COUNT",
                f"Verify Daily Maintenance country universe count for {country}",
                ...
                evidence_file="memberships_daily_country_count_diffs.csv" if not count_match else "",
            )
            ...
            for country_col in COUNTRY_COLUMNS:
                ...
                self.add_check(
                    f"MEM-DAILY-{country}-{country_col}",
                    ...
                    evidence_file=(
                        "memberships_daily_country_field_diffs.csv"
                        if mismatch_count > 0
                        else ("memberships_daily_invalid_country_values.csv" if invalid_count > 0 else "")
                    ),
                )
            ...
            self.add_check(
                ...
                evidence_file="memberships_daily_country_alignment_issues.csv" if alignment_failures > 0 else "",
            )

        if field_diff_records:
            self.evidence["memberships_daily_country_field_diffs"] = pd.DataFrame(field_diff_records)
        if internal_alignment_issues:
            self.evidence["memberships_daily_country_alignment_issues"] = pd.DataFrame(internal_alignment_issues)
        if country_count_diffs:
            self.evidence["memberships_daily_country_count_diffs"] = pd.DataFrame(country_count_diffs)
        if invalid_country_values:
            self.evidence["memberships_daily_invalid_country_values"] = pd.DataFrame(invalid_country_values)
        ...
        self.add_check(
            "MEM-DAILY-006Z",
            "Verify all country fields contain recognized country values",
            ...
            evidence_file="memberships_daily_invalid_country_values.csv" if overall_validity_issues > 0 else "",
        )

    def _validate_null_blank(self, inf_df: pd.DataFrame) -> None:
        required = list(dict.fromkeys(KEY_COLUMNS_PRIMARY + KEY_COLUMNS_SECONDARY + COUNTRY_COLUMNS))
        for col in required:
    ...
    def _validate_duplicate_behavior(self, domo_df: pd.DataFrame, inf_df: pd.DataFrame) -> None:
        for idx, col in enumerate(KEY_COLUMNS_PRIMARY + KEY_COLUMNS_SECONDARY, start=1):
            if col not in domo_df.columns or col not in inf_df.columns:
                continue

            domo_counts = Counter(self._trim_series(domo_df[col]))
            inf_counts = Counter(self._trim_series(inf_df[col]))

            diffs = []
            for value in set(domo_counts) | set(inf_counts):
            ...
            if diffs:
                self.evidence[f"memberships_daily_duplicate_diff_{col}"] = pd.DataFrame(diffs[:MAX_EVIDENCE_ROWS])

            self.add_check(
                f"MEM-DAILY-009-DUP-{idx}",
                f"Verify duplicate behavior for {col}",
                "PASS" if not diffs else "FAIL",
                "Population",
                field=col,
                expected="Same multiplicity per value",
                actual="Matched" if not diffs else f"duplicate_differences={len(diffs)}",
                evidence_file=f"memberships_daily_duplicate_diff_{col}.csv" if diffs else "",
            )
